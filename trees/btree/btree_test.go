...
func TestBTreeGet1(t *testing.T) {
	tree := NewWithIntComparator[string](3)
	tree.Put(1, "a")
	tree.Put(2, "b")
	tree.Put(3, "c")
	tree.Put(4, "d")
	...
	for _, test := range tests {
		if value, found := tree.Get(test.key); value != test.val || found != test.expect {
			t.Errorf("Got %v,%v expected %v,%v", value, found, test.val, test.expect)
		}
	}
}
...
func TestBTreePut1(t *testing.T) {
	...
	tree := NewWithIntComparator[int](3)
...
func TestBTreePut2(t *testing.T) {
	tree := NewWithIntComparator[int](4)
	assertValidTree(t, tree, 0)
...
func TestBTreePut3(t *testing.T) {
	...
	tree := NewWithIntComparator[int](6)
	assertValidTree(t, tree, 0)
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	tree := NewWithIntComparator[string](4)
	tree.Put(4, "d")
	tree.Put(5, "e")
	tree.Put(6, "f")
	tree.Put(3, "c")
	...
	if actualValue, expectedValue := concatenate(tree.Values()...), "xbcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestBTree_search(t *testing.T) {
	{
		tree := NewWithIntComparator[int](3)
		tree.Root = &Node[int, int]{
			Entries:  []*Entry[int, int]{},
			Children: make([]*Node[int, int], 0),
		}
		tests := [][]interface{}{
			{0, 0, false},
		}
		for _, test := range tests {
			...
			if actualValue, expectedValue := found, test[2].(bool); actualValue != expectedValue {
				t.Errorf("Got %v expected %v", actualValue, expectedValue)
			}
		}
	}
	{
		...
		for _, test := range tests {
			...
			if actualValue, expectedValue := index, test.val; actualValue != expectedValue {
				t.Errorf("Got %v expected %v", actualValue, expectedValue)
			}
			if actualValue, expectedValue := found, test.found; actualValue != expectedValue {
				t.Errorf("Got %v expected %v", actualValue, expectedValue)
			}
		}
	}
...
func assertValidTree[K comparable, V comparable](t *testing.T,
	tree *Tree[K, V], expectedSize int) {
...
func assertValidTreeNode[V comparable](t *testing.T,
	node *Node[int, V], expectedEntries int, expectedChildren int, keys []int, hasParent bool) {
...
func TestBTreeSerialization(t *testing.T) {
	tree := NewWithStringComparator[string](3)
	tree.Put("c", "3")
	tree.Put("b", "2")
	tree.Put("a", "1")
...
func BenchmarkBTreeGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	tree := NewWithIntComparator[empty](128)
	for n := 0; n < size; n++ {
		tree.Put(n, emptyVal)
	}
	b.StartTimer()
