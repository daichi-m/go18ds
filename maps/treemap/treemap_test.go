...
func TestMapFloor(t *testing.T) {
	...
	for _, test := range tests1 {
		// retrievals
		actualKey, actualValue := m.Floor(test.key)
		actualFound := actualKey != 0 && actualValue != ""
		if actualKey != test.exKey || actualValue != test.exVal || actualFound != test.found {
			t.Errorf("Got %v, %v, %v, expected %v, %v, %v", actualKey, actualValue, actualFound, test.exKey, test.exVal, test.found)
		}
	}
}
...
func TestMapCeiling(t *testing.T) {
	...
	for _, test := range tests1 {
		// retrievals
		actualKey, actualValue := m.Ceiling(test.key)
		actualFound := actualKey != 0 && actualValue != ""
		if actualKey != test.exKey || actualValue != test.exVal || actualFound != test.found {
			t.Errorf("Got %v, %v, %v, expected %v, %v, %v", actualKey, actualValue, actualFound, test.exKey, test.exVal, test.found)
		}
...
func TestMapSerialization(t *testing.T) {
	for i := 0; i < 10; i++ {
		original := NewWithStringComparator[string]()
		original.Put("d", "4")
		original.Put("e", "5")
		original.Put("c", "3")
		original.Put("b", "2")
		...
		deserialized := NewWithStringComparator[string]()
